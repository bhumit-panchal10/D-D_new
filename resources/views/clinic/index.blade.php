                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            ...
                            justify-content: space-between;">
                                <h5 class="card-title mb-0">Clinic List</h5>
                                <a href="{{ route('clinic.create') }}" class="btn btn-sm btn-primary">
                                ...
                                <table id="scroll-horizontal" class="table nowrap align-middle" style="width : 100%">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($clinics as $clinic)
                                            <tr>
                                                <td>{{ $i + $clinics->perPage() * ($clinics->currentPage() - 1) }}</td>
                                                <td>{{ $clinic->name }}</td>
                                                <td>{{ $clinic->doctor }}</td>
                                                <td>{{ $clinic->mobile_no }}</td>
                                                <td>{{ $clinic->start_date ? \Carbon\Carbon::parse($clinic->start_date)->format('d-m-Y') : '-' }}
                                                </td>
                                                <td>{{ $clinic->end_date ? \Carbon\Carbon::parse($clinic->end_date)->format('d-m-Y') : '-' }}
                                                </td>
                                                ...
                                                <td style="display: flex">
                                                    @if ($clinic->iStatus == 0)
                                                        <a href="{{ route('clinic.status', ['clinic_id' => $clinic->clinic_id, 'status' => 1]) }}"
                                                            title="InActive" class="mx-1">
                                                            <i class="fa fa-lock" aria-hidden="true"></i>
                                                        </a>
                                                    @elseif ($clinic->iStatus == 1)
                                                        <a href="{{ route('clinic.status', ['clinic_id' => $clinic->clinic_id, 'status' => 0]) }}"
                                                            title="Active" class="mx-1">
                                                            <i class="fa fa-unlock" aria-hidden="true"></i>
                                                        </a>
                                                    @endif
                                                    <a class="mx-1" title="Edit"
                                                        href="{{ route('clinic.edit', $clinic->clinic_id) }}">
                                                        <i class="far fa-edit"></i>
                                                    </a>
                                                    <a class="mx-1" href="#" data-bs-toggle="modal" title="Delete"
                                                        data-bs-target="#deleteRecordModal"
                                                        onclick="deleteData(<?= $clinic->clinic_id ?>);">
                                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                                    </a>
                                                    <a class="mx-1" href="#" data-bs-toggle="modal"
                                                        title="Change Password" data-bs-target="#changepassword"
                                                        onclick="editpassword(<?= $clinic->clinic_id ?>);">
                                                        <i class="fa fa-key" aria-hidden="true"></i>
                                                    </a>
                                                    <a class="mx-1" href="#" data-bs-toggle="modal"
                                                        title="Order Create" data-bs-target="#ordercreate"
                                                        onclick="ordercreate(<?= $clinic->clinic_id ?>);">
    ...
    <div class="modal fade" id="changepassword" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('clinic.password_update') }}" autocomplete="off">
                    @csrf
                    @method('post')
    ...
    <div class="modal fade" id="ordercreate" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('clinic.ordercreate') }}" autocomplete="off">
                    @csrf
                    @method('post')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                        ...
                        <form id="user-delete-form" method="POST" action="{{ route('clinic.destroy') }}">
                            @csrf
                            @method('DELETE')
